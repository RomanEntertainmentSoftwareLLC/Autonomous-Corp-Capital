# MEMORY.md

_Agent-local durable memory._

## Evaluation lessons
- 2026-04-25: [axiom_review] High XP is not trustworthy by itself; compare reward signals against evidence-quality scores.
- 2026-04-25: [axiom_review] Partial portfolio coverage plus imputed display equity is a gate blocker for honest paper-test evaluation.
- 2026-04-25: [axiom_review] Executed decisions with zero ML coverage and zero trace rows should trigger audit concern immediately.
- 2026-04-26: [axiom_review] Fresh committee packets do not compensate for missing portfolio coverage and unknown deployable equity.
- 2026-04-26: [axiom_review] Executed decisions without ML scores, evidence winners, or trace rows should be treated as weakly auditable.
- 2026-04-26: [axiom_review] Reward logic needs cross-checking against evidence-quality scores, not just sessions, usefulness, or output volume.
