...
def choose_adapter(agent_info: Dict[str, Any]) -> SimpleLLMAdapter | OpenAIAdapter:
    kind = agent_info.get("agent_kind", "company_local")
    if kind in ("global", "swe", "watchdog", "company_local"):
        try:
...
def main() -> None:
    ...
    adapter = choose_adapter(agent_info)
